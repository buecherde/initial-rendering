 ...
 * @param {object} [options.waitFor] - waitFor n ms after each step
 * @param {number} [options.waitFor.1=0] - delay after step 1
 * @param {number} [options.waitFor.2=0] - delay after step 2
 * @param {number} [options.waitFor.3=0] - delay after step 3
 * @returns {promise.<initialRenderingResult>}
 */
const initialRendering = ({url=false, device='Nexus 5X', puppeteerOptions={}, auth={}, returnScreenshots=true, waitFor={}} = {}) => {
	...
	return Promise.all([
		getScreenshot('initial', waitFor[1] || 0),
		getScreenshot('no-assets', waitFor[2] || 0),
		getScreenshot('full', waitFor[3] || 0)
	 ...
	 */
	async function getScreenshot(type, waitFor) {
		const result = [
			type
		];
		...
		await page.goto(url);
		if (waitFor != null && typeof waitFor === 'number' && waitFor > 0) {
