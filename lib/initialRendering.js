 ...
 */
const initialRendering = ({url=false, device='Nexus 5X', puppeteerOptions={}, auth={}} = {}) => {
	...
	return Promise.all([
		...
		for (let i in screenshots) {
			...
			const msg = `${screenshot[0]}:${last[0]} ${numDiffPixels} ${ratio}%`;

			result.steps.push({
				name: screenshot[0],
				numDiffPixels,
				ratio,
				screenshot: await pngToBase64(screenshot[1]),
				diff: await pngToBase64(diff)
			});
		}
